...
class LlmService
  ...
  def process
    ...
    if chat_gpt_version != -1
      ...
      elsif chat_gpt_version == 4
        chunks = @resource_data.map { |r| r[:chunk] }.join(', ')
        @llm_instructions = 'You must use only this text in parentheses to prepare your answer: (' + chunks + ')  You must notify the user if you cannot. You must never make up an answer on your own. Here is my question: '
      end
      ...
      begin
        ...
        @logger.error "POST request to #{@g.endpoint} failed: #{e.message}"
        stored_response = "We are sorry, but the remote service did not respond"
        res_status = 200
      end

      @ai.response = stored_response
      @ai.http_status = res_status
  ...
  def prevent_overflow
    ...
    if (!@window_size.nil?) && (@window_size.is_a? Numeric) && (resources + volleys + base > @window_size)
